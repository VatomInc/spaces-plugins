...
* Armor Pickup
*
* Allows for creation of armor pickups
*
* @license MIT
* @author Liron-Toledo
*/
module.exports = class ArmorPlugin extends BasePlugin {

    /** Plugin info */
    static get id()             { return 'armor-pickup-plugin' }
    static get name()           { return 'Armor Pickup' }
    static get description()    { return 'Allows for creation of armor pickups' }

    /** Called when the plugin is loaded */
    onLoad() {

        this.objects.registerComponent(ArmorPickup, {
            id: 'armor-pickup',
            name: 'Armor Pickup',
            description: 'Converts this object into an armor pickup.',
            settings: [
                ...
                { id: 'amount', name: 'Armor Amount', type: 'number', default: 25, help: 'How much armor is gained from this pickup' },
...
class ArmorPickup extends BaseComponent {

    onLoad() {

        const sound = this.paths.absolute('./Armor_Pickup.mp3')
        if (sound) {
    ...
    async onMessage(e) {
        if (e.id != this.instanceID) {
            if(e.action == "activate-pickup"){
                // this.plugin.audio.play(absolutePath('Armor_Pickup.mp3'), {x: e.position.x, y: e.position.z, height: e.position.y, radius: 40})
    ...
    async onPickup() {
        ...
        await this.plugin.hooks.trigger('fpshud.armor.change', {amount: amount})
