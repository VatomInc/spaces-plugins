...
* Health Pickup
*
* Allows for creation of health pickups
*
* @license MIT
* @author Liron-Toledo
*/
module.exports = class HealthPlugin extends BasePlugin {

    /** Plugin info */
    static get id()             { return 'health-pickup-plugin' }
    static get name()           { return 'Health Pickup' }
    static get description()    { return 'Allows for creation of health pickups' }

    /** Called when the plugin is loaded */
    onLoad() {

        this.objects.registerComponent(HealthPickup, {
            id: 'health-pickup',
            name: 'Health Pickup',
            description: 'Converts this object into a health pickup.',
            settings: [
                ...
                { id: 'amount', name: 'Health Amount', type: 'number', default: 25, help: 'How much health is gained from this pickup' },
...
class HealthPickup extends BaseComponent {

    onLoad() {

        const sound = this.paths.absolute('./Health_Pickup.mp3')
        if (sound) {
    ...
    async onMessage(e) {
        if (e.id != this.instanceID) {
            if(e.action == "activate-pickup"){
                // this.plugin.audio.play(absolutePath('Health_Pickup.mp3'), {x: e.position.x, y: e.position.z, height: e.position.y, radius: 40})
